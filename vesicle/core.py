...
class Point(geometry.Point):
    def __init__(self, x=None, y=None, ptype="", profile=None):
        ...
        self.dist_to_nearest_vesicle_center = None
        self.dist_to_nearest_vesicle_border = None
        self.is_within_profile = None
        self.is_associated_with_path = None
        self.is_associated_with_profile = None
        self.is_within_vesicle = None
        self.is_associated_with_vesicle = None
    ...
    def determine_stuff(self, profile):
        ...
        self.is_within_profile = self._is_within_profile()
        ...
        self.dist_to_nearest_vesicle_center = \
            self._dist_to_nearest_vesicle_center()
        self.dist_to_nearest_vesicle_border = \
            self._dist_to_nearest_vesicle_border()
        self.is_within_vesicle = self._is_within_vesicle()
        self.is_associated_with_vesicle = (self.dist_to_nearest_vesicle_border
                                           <= geometry.to_pixel_units(
                                           profile.opt.spatial_resolution,
                                           profile.pixelwidth))

    def _is_within_vesicle(self):
    ...
    def _dist_to_nearest_vesicle_center(self):
    ...
    def _dist_to_nearest_vesicle_border(self):
    ...
    def _is_within_profile(self):
        """Determine if self is inside profile, excluding holes."""
        if ((not self.is_within_polygon(self.profile.path))
                or self.__is_within_hole()):
            return False
        return True
...
class ProfileBorderData(geometry.SegmentedPath):
    ...
    def area(self):
        ...
        tot_hole_area = sum([h.area() for h in self.profile.holeli])
...
class ProfileData:
    ...
    def process(self):
        ...
        try:
            ...
            if self.opt.determine_intervesicle_dists:
                self.get_interdistlis("vesicle")
            if self.opt.determine_interpoint_dists:
                self.get_interdistlis("point")
    ...
    def get_interdistlis(self, comp):
        if comp == "point":
            compli = self.pli
        elif comp == "vesicle":
        ...
        if rel_dict["%s - %s" % (comp, comp)]:
            self.interdistlis["%s - %s" % (comp, comp)] = \
                self.get_same_interdists(compli, dist_mode, shortest_dist,
                                         lateral_dist)
        if self.opt.use_random and rel_dict["random - %s" % comp]:
            self.interdistlis["random - %s" % comp] = \
                self.get_interdists(self.randomli, compli, dist_mode,
    ...
    def get_same_interdists(self, pointli, dist_mode, shortest_dist,
                            lateral_dist):
                ...
                for j in range(i + 1, len(pointli)):
                    ...
                    if lateral_dist:
                        distdict["lateral"].append(
                            pointli[i].lateral_dist_to_point(pointli[j],
                                                             self.path))
    ...
    def get_interdists(self, pointli, pointli2, dist_mode, shortest_dist,
                       lateral_dist):
        if pointli2 is None:
            pointli2 = []
        dli = []
    ...
    def __check_parsed_data(self):
        ...
        self._check_list_var('path', 'Profile border', 'nodes', 2)
        self._check_list_var('pli', 'Points', '', 0)
        self._check_table_var('vli', "Vesicle", "Vesicles", 1, 2)
    ...
    def _check_var_exists(self, var_to_check, var_str, optflag):
        ...
        if self.opt.__dict__[optflag] is None:
            if self.__dict__[var_to_check] not in (None, []):
        ...
        if self.opt.__dict__[optflag]:
            if self.__dict__[var_to_check] not in (None, []):
                ...
                raise ProfileError(self, "%s not found in input file" % var_str)
        elif self.__dict__[var_to_check] not in (None, []):
    ...
    def __check_paths(self):
        ...
        def check_simple(_path, s):
            if not _path.is_simple_polygon():
                raise ProfileError(self, "%s is invalid (crosses itself)" %
                                         s.capitalize())

        def check_in_profile(_path, s):
            if not _path.is_within_polygon(self.path):
                raise ProfileError(self, "%s is not within the profile "
                                         % s.capitalize())
            ...
            for n, p in enumerate(pathli):
                ...
                if s_other != "profile border":
                    ...
                    if otherpath.is_within_polygon(p):
                        raise ProfileError(self, "A %s is within %s %d"
                                                 % (s, s_other, n + 1))
...
class OptionData:
    def __init__(self):
        ...
        self.determine_point_vesicle_center_dist = False
        self.determine_point_vesicle_border_dist = False
